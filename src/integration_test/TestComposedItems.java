	...
	public static void main(String[] args) throws SlickException {
		...
		PlayerBuilder playerBuilder = new AbstractPlayerBuilder() {
			...
			protected void updatePlayer(Player player) {
				player.setPosition(Position.fromCoordinates(268, 32, MapType.ALTERNATIVE));
				player.setMaxLife(100);
				player.setLife(100);

				List<String> debuggerPositions = new ArrayList<>();
				debuggerPositions.add(String.format("%s/%s/%s", 268, 28, MapType.ALTERNATIVE));
				debuggerPositions.add(String.format("%s/%s/%s", 272, 28, MapType.ALTERNATIVE));
