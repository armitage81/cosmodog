...
public class ChartInventoryItem extends CountableInventoryItem {
	...
	public ChartInventoryItem() {
		...
		for (int i = 0; i < ACTUAL_CHART_PIECE_NUMBER_X * ACTUAL_CHART_PIECE_NUMBER_Y; i++) {
	...
	public boolean pieceIsDiscovered(int x, int y) {
		Preconditions.checkState(x < ACTUAL_CHART_PIECE_NUMBER_X);
		Preconditions.checkState(y < ACTUAL_CHART_PIECE_NUMBER_Y);
		int index = y * ACTUAL_CHART_PIECE_NUMBER_Y + x;
